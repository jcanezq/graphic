...
const outFile = 'D:\\proyectos\\galeria\\cotigrafic\\supabase\\_recon\\2026-09-09-recon.md';
let output = "# Reconnaissance Results\n\n";

for (let i = 0; i < queries.length; i++) {
  const q = queries[i];
  output += `## Query ${i + 1}\n\`\`\`sql\n${q}\n\`\`\`\n\n\`\`\`json\n`;
  try {
    const res = execSync(`npx supabase db query "${q}" --linked`, { encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] });
    output += res;
  } catch (err) {
    output += err.stdout;
    output += err.stderr;
